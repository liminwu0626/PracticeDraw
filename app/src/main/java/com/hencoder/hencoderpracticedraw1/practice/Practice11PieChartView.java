...
public class Practice11PieChartView extends View {
    ...
    private float lineStartX = 0f;//直线开始的 X 坐标
    private float lineStartY = 0f;//直线开始的 Y 坐标
    private float lineEndX;//直线结束的 X 坐标
    private float lineEndY;//直线结束的 Y 坐标
...
//        练习内容：使用各种 Canvas.drawXXX() 方法画饼图
        canvas.translate(canvas.getWidth() / 2, canvas.getHeight() / 2);//将画布（0，0）坐标移动到画布的中心
        startAngle = 0f;
        for (Data data : datas) {
            ...
            if (lineAngle > 90 && lineAngle <= 270) {
                ...
                canvas.drawLine(lineEndX, lineEndY, lineEndX + 50, lineEndY, paint);
                paint.setStyle(Paint.Style.FILL);
                canvas.drawText(data.getName(), lineEndX + 50, lineEndY, paint);
            }
            if (data.getNumber() == max) {
                canvas.restore();
            }
